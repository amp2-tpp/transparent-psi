...
const renderFinish = () => {
  ...
  if (neededReward) {
    if(server.user.session_type === 'online') {
      ...
      domInjector("h4", ".intro", (texts.rewardOnlineLink.replace('https://survey.maximiles.com/static-complete?p=75746_0193c42d', (texts.rewardOnlineLink.split("'")[5].concat(`&m=${server.user.BilendiID}`)))))
    } else {
...
const refuse = (param) => {
  return () => {
    ...
    domInjector("h4", ".intro", (texts.refuse.replace('https://survey.maximiles.com/static-screenout?p=75746', (texts.refuse.split("'")[5].concat(`&m=${server.user.BilendiID}`)))))
  }
}
